...
class CheckpointHook(Hook):
    ...
    def _save_best_checkpoint(self, runner, metrics) -> None:
        ...
        ############################
        if not hasattr(self, 'best_ckpt_paths'):
            self.best_ckpt_paths = []
        ############################
        ...
        for key_indicator, rule in zip(self.key_indicators, self.rules):
            ...
            # 我自己的实现, 保存top k的模型
            # 新增部分: 更新 topk 模型列表
            topk_ckpt_name = f'top_{key_indicator}_{key_score:.4f}_{ckpt_filename}'
            topk_ckpt_path = self.file_backend.join_path(self.out_dir, topk_ckpt_name)
            
            # save_top_k = min(self.save_top_k, self.max_keep_ckpts) if self.max_keep_ckpts > 0 else self.save_top_k  # 这个逻辑是考虑到 max_keep_ckpts<save_top_k的情况,要以max_keep_ckpts为准
            save_top_k = self.save_top_k
            
            if len(self.best_ckpt_paths) < save_top_k:
                # 如果 best_ckpt_paths 的长度还没有达到 save_top_k,直接将当前权重添加到 best_ckpt_paths 中
                self.best_ckpt_paths.append((topk_ckpt_path, key_score))
                self.best_ckpt_paths.sort(key=lambda x: x[1], reverse=self.rule_map[rule](1, 0))
                runner.save_checkpoint(
                    self.out_dir,
                    filename=topk_ckpt_name,
                    ...
                    backend_args=self.backend_args)
                runner.logger.info(
                    f'The top{len(self.best_ckpt_paths)} checkpoint with {key_score:0.4f} {key_indicator} '
                    f'at {cur_time} {cur_type} is saved to {topk_ckpt_name}.')
            else:
                # 如果 best_ckpt_paths 的长度已经达到 save_top_k,比较当前权重的指标值与 best_ckpt_paths 中的最后一名
                last_score = self.best_ckpt_paths[-1][1]
                runner.logger.info(
                    f'Current {key_indicator} score: {key_score:.4f}, '
                    f'last score in topk: {last_score:.4f}')
                if key_score > last_score:
                    # 如果当前权重的指标值大于 best_ckpt_paths 中的最后一名,则将最后一名删除,并将当前权重添加到 best_ckpt_paths 中
                    old_ckpt_path = self.best_ckpt_paths.pop(-1)[0]
                    if is_main_process() and self.file_backend.exists(old_ckpt_path):
                        self.file_backend.remove(old_ckpt_path)
                    self.best_ckpt_paths.append((topk_ckpt_path, key_score))
                    self.best_ckpt_paths.sort(key=lambda x: x[1], reverse=self.rule_map[rule](1, 0))
                    runner.save_checkpoint(
                        self.out_dir,
                        ...
                        backend_args=self.backend_args)
                    runner.logger.info(
                        f'The top{save_top_k} checkpoint with {key_score:0.4f} {key_indicator} '
                        f'at {cur_time} {cur_type} is saved to {topk_ckpt_name}.')
                else:
                    runner.logger.info(
                        f'The current {key_indicator} score {key_score:.4f} is no better than '
                        f'the last score in topk {last_score:.4f}, no need to save.')
            ############################
